...
$("#add-project").on("click", ".structure", function () {
  $(".structure-container").removeClass("hide");
});

// đóng
$(".structure-mockup").on("click", ".struc-close", function () {
  $(".structure-container").addClass("hide");
});
...
$(".container").on("click", ".add-button", function () {
  ...
  var level =
    parseInt(parentBlock.attr("class").match(/block-level-(\d+)/)[1]) + 1;
  parentBlock.append(
    '<input type="text" class="add-input" placeholder="Nhập tên cho Cấp Độ ' +
      level +
      '">'
  );
  parentBlock.append(
    '<input type="text" class="add-code" placeholder="Nhập mã cho Cấp Độ ' +
      level +
      '">'
  ); // Thêm input cho mã vào khối cha
  ...
  parentBlock
    .children(".block-level-" + (level - 1) + " > .block")
    .each(function () {
      if (!$(this).is(":visible")) {
