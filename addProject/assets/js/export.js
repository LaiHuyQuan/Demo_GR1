$(document).ready(function () {
  ...
  $("#add-project").on("click", ".save", function () {
    ...
    // Lấy dữ liệu từ projectData và jsonStructure
    // var projectDataJson = JSON.stringify(projectData, null, 2);
    var jsonStructureJson = JSON.stringify(jsonStructure, null, 2);

    var combinedJson = jsonStructureJson;

    // Tải xuống tệp văn bản
    download("projectData.txt", combinedJson);
  });
    ...
    projectData.Devices.forEach(function (device) {
      ...
      device.Channels.forEach(function (channel) {
        ...
        if (channel.chonpha == "3 pha") {
          ...
          channels.push(
            Object.assign({}, channelData, { chonch: channel.chonch })
          );
        }
        loadId++;
      });
