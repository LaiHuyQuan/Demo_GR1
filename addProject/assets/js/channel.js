...
$(".add-channel-mockup").on("click", ".channel-save", function () {
  ...
  $("#main")
    ...
    .append(createchannelTable(deviceID));
  ...
  $(".channel-save").remove();
  $(".channel-container").addClass("hide");
});
...
$(".add-device").on("click", ".edit-channel-btn", function () {
  ...
  var channelSavebtn =
    '<button type="" class="btn btn-primary me-1 mb-1 edit-channel-save" data-info="' +
    deviceID +
    '" data-chonch="' +
    channelID +
    '">Save</button>';
  ...
  for (var i = 0; i < projectData.devices.length; i++) {
    ...
    for (var j = 0; j < device.channels.length; j++) {
      ...
      if (channel.chonch === channelID) {
        ...
        channel.lv1code = $(".level1-inputs")
          .find("option:selected")
          .data("code");
        channel.lv2code = $(".level2-inputs")
          .find("option:selected")
          .data("code");
        channel.lv3code = $(".level3-inputs")
          .find("option:selected")
          .data("code");
        channel.lv4code = $(".level4-inputs")
          .find("option:selected")
          .data("code");
      }
    }
...
function addChannel(deviceID, chonch, chonpha) {
  ...
  $(".add-channel-mockup")
    ...
    .text("Channel " + chonch)
    .data("info", chonch)
    .data("chonpha", chonpha);
  var channelSavebtn =
    '<button type="reset" class="btn btn-primary me-1 mb-1 channel-save" data-info="' +
    deviceID +
    '">Save</button>';
  $(".channel-btns").append(channelSavebtn);
}
